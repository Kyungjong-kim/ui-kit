...
**주의**: `dist/styles.css`는 **디자인 토큰만** 포함 (107줄). 컴포넌트 유틸리티 클래스는 사용처(외부 앱)의 Tailwind v4가 ui-kit dist를 스캔해야 생성됨.
...
├── components/
│   ├── index.ts              # primitives 재export
│   └── primitives/
│       ├── index.ts          # 22개 컴포넌트 export
│       ├── accordion/
│       │   ├── accordion.tsx
│       │   ├── accordion.test.tsx
│       │   └── index.ts
...
```
src/components/primitives/<kebab-case>/
├── <kebab-case>.tsx          # 컴포넌트 본체
├── <kebab-case>.test.tsx     # vitest 단위 테스트
└── index.ts                  # export 재노출

stories/<kebab-case>.stories.tsx
src/components/primitives/index.ts  # ← export 추가 갱신 필수
```
...
|---|---|---|
| Radix 래핑 (Trigger·Content 분리) | ✅ 사용 | `Accordion`, `Dialog`, `Tabs`, `DropdownMenu` |
| 단순 props 전달 함수 | ❌ 미사용 (함수 선언) | `Avatar`, `Spinner`, `Skeleton`, `EmptyState` |
| Button 류 (DOM ref 노출 필요) | ✅ 사용 | `Button` |
...
[테스트] pnpm test (vitest run)  →  22 files / 54 tests
...
에이전트는 컴포넌트 작업 시:
1. 수정하려는 컴포넌트가 **어느 카테고리**(Radix 래핑 / 단순 / DnD)인지 먼저 분류
2. 동일 카테고리 컴포넌트 2개 이상 읽고 패턴 답습
