...
describe("AreaChart", () => {
  it("크래시 없이 루트 컨테이너를 렌더하고 높이를 반영한다", () => {
    const { container } = render(
      <AreaChart data={data} xKey="month" series={[{ dataKey: "매출" }]} height={240} />,
    );
  ...
  it("className을 병합한다", () => {
    const { container } = render(
      <AreaChart data={data} xKey="month" series={[{ dataKey: "매출" }]} className="custom-cls" />,
    );
    expect(container.firstElementChild).toHaveClass("custom-cls");
  });

  it("시리즈 개수만큼 area를 그린다", () => {
    const { container } = render(
      <AreaChart data={data} xKey="month" series={[{ dataKey: "매출" }, { dataKey: "비용" }]} />,
    );
